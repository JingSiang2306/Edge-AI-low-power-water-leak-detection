/**
 * vibration_detection.c
 *
 * NanoEdge AI-based vibration leak detection (sliding-window version)
 */
...
#define VIBRATION_SLOT_DURATION_MS                    500U
#define VIBRATION_MAX_SLOT_COUNT                     16U
#define VIBRATION_STRONG_LEAK_CONFIDENCE_THRESHOLD   0.80f
#define VIBRATION_SLOT_LEAK_CONFIDENCE_THRESHOLD     0.60f
#define VIBRATION_MIN_LEAKY_SLOT_COUNT               2U
#define VIBRATION_STRONG_LEAK_FRACTION_THRESHOLD     0.35f

uint32_t g_vneai_num_windows         = 30U;
uint8_t  g_vneai_skip_first_window   = 0U;

#if VIB_NEAI_ENABLE

/* IMPORTANT: include shim BEFORE NanoEdgeAI header so prototypes get remapped */
#include "NEAI_Vibration/NEAI_vibration_shim.h"
#include "NEAI_Vibration/NanoEdgeAI_vibration.h"
...
static float vibration_neai_input_buffer[DATA_INPUT_USER * AXIS_NUMBER];
static float vibration_neai_output_probabilities[CLASS_NUMBER];

/* Resolved 0-based NEAI class IDs for this vibration model */
static int g_vibration_neai_leak_class_id = 0;
static int g_vibration_neai_background_class_id = 1;

static const char *vibration_get_class_name_from_predicted_id(uint16_t predicted_class_id)
{
...
static const char *vibration_get_class_name_from_neai_id(int neai_class_id)
{
    const char *class_name = neai_get_class_name(neai_class_id);
    return (class_name != NULL) ? class_name : "Unknown";
}

static float vibration_get_max_probability(const float *class_probabilities)
{
...
static void vibration_neai_resolve_class_ids(void)
{
    int number_of_classes = neai_get_number_of_classes();
    if (number_of_classes <= 0) {
        g_vibration_neai_leak_class_id = 0;
        g_vibration_neai_background_class_id = 1;
        printf("[NEAI] Vibration class mapping: invalid class count, fallback to Leak=0, Background=1\r\n");
        return;
    }
    ...
    g_vibration_neai_leak_class_id = resolved_leak_class_id;
    g_vibration_neai_background_class_id = resolved_background_class_id;

    printf("[NEAI] Vibration class mapping resolved:\r\n");
    for (int class_index = 0; class_index < number_of_classes; class_index++) {
        printf("[NEAI]   model_id=%d -> %s%s%s\r\n",
               class_index,
               vibration_get_class_name_from_neai_id(class_index),
               (class_index == g_vibration_neai_leak_class_id) ? " [used as Leak]" : "",
               (class_index == g_vibration_neai_background_class_id) ? " [used as Background]" : "");
    }
}

#endif /* VIB_NEAI_ENABLE */
...
static void vibration_print_window_with_time(uint16_t predicted_class_id,
                                             ...
                                             uint32_t sample_rate_hz)
{
...
#else
    uint32_t center_samples = offset_samples;
#endif
    uint32_t window_ms      = (sample_rate_hz > 0U) ? ((center_samples * 1000U) / sample_rate_hz) : 0U;
    ...
    printf("[NEAI] win=%lu \t\t time: %s \t\t offset=%lu \t\t\t class_id=%u (%s)\t\t\t\t probs=[",
           ...
           vibration_get_class_name_from_predicted_id(predicted_class_id));

    float highest_probability = vibration_get_max_probability(class_probabilities);
    ...
    printf("%.3f, %.3f]\r\n",
           (double)class_probabilities[0],
           (double)class_probabilities[1]);
}
...
void vibration_detection_init(void)
{
    ...
    if (initialization_status != NEAI_OK) {
        printf("[NEAI] Vibration detection initialize - error: %d\r\n", (int)initialization_status);

    } else {
        vibration_neai_resolve_class_ids();
        printf("[NEAI] Vibration detection initialize - OK\r\n");
    }
#else
    printf("[NEAI] Vibration model disabled (VIB_NEAI_ENABLE=0). Using stub.\r\n");
#endif
}
                                                        ...
                                                        det_3s_result_t *out_res)
{
    ...
    if (z_samples == NULL || num_samples < 16U) {
        return NEAI_DET_ERR_ARGS;
    }

#if !VIB_NEAI_ENABLE
    if (predicted_class_id) {
        *predicted_class_id = 2U;
    }
    if (out_res) {
        out_res->predicted_class_id = 2U;
        out_res->predicted_is_leak  = 0U;
        out_res->average_leak_confidence       = 0.0f;
        out_res->average_background_confidence         = 1.0f;
    }
    printf("[NEAI] Stub: no vibration inference. Treating as background.\r\n");
    (void)sample_rate_hz;
    (void)record_time_ms_of_day;
    (void)record_date_str;
    return NEAI_DET_OK;
#else

    if (num_samples < DATA_INPUT_USER) {
        return NEAI_DET_ERR_ARGS;
    }
    ...
    if (num_samples > DATA_INPUT_USER) {
        max_possible_windows = (num_samples - DATA_INPUT_USER) + 1U;
    }

    uint32_t num_windows = g_vneai_num_windows;
    if (num_windows < 1U) {
        num_windows = 1U;
    }
    ...
    uint32_t hop = 0U;
    if (num_windows > 1U) {
        hop = (num_samples - DATA_INPUT_USER) / (num_windows - 1U);
    }

    uint32_t total_ms = (sample_rate_hz > 0U)
                      ? (uint32_t)((num_samples * 1000U + (sample_rate_hz - 1U)) / sample_rate_hz)
                      : 0U;

    uint32_t slot_count = (total_ms + (VIBRATION_SLOT_DURATION_MS - 1U)) / VIBRATION_SLOT_DURATION_MS;
    if (slot_count == 0U) {
        slot_count = 1U;
    }
    if (slot_count > VIBRATION_MAX_SLOT_COUNT) {
        slot_count = VIBRATION_MAX_SLOT_COUNT;
    }

    float    slot_leak_sum[VIBRATION_MAX_SLOT_COUNT]            = {0.0f};
    float    slot_bg_sum[VIBRATION_MAX_SLOT_COUNT]              = {0.0f};
    uint32_t slot_counts[VIBRATION_MAX_SLOT_COUNT]              = {0U};
    uint32_t slot_strong_leak_window_count[VIBRATION_MAX_SLOT_COUNT] = {0U};
    ...
    const int32_t baseline_z = (int32_t)z_samples[0];

    neai_det_status_t overall_status = NEAI_DET_OK;

    for (uint32_t w = 0U; w < num_windows; w++) {
        uint32_t offset = w * hop;
        if (offset + DATA_INPUT_USER > num_samples) {
            offset = num_samples - DATA_INPUT_USER;
        }

        for (uint32_t i = 0U; i < DATA_INPUT_USER; i++) {
            int32_t dz = (int32_t)z_samples[offset + i] - baseline_z;
            vibration_neai_input_buffer[i] = (float)dz;
        }

        int neai_id0 = 0;
        enum neai_state err = neai_classification(vibration_neai_input_buffer,
        		vibration_neai_output_probabilities,
                                                  &neai_id0);
        if (err != NEAI_OK) {
        ...
        if (neai_id0 == g_vibration_neai_leak_class_id) {
            id_class_1b = 1U;
        } else if (neai_id0 == g_vibration_neai_background_class_id) {
            id_class_1b = 2U;
        } else {
            id_class_1b = 0U;
        }

        uint8_t  id_valid    = (id_class_1b >= 1U && id_class_1b <= 2U);
        uint8_t  is_leak_id  = (neai_id0 == g_vibration_neai_leak_class_id) ? 1U : 0U;

        vibration_print_window_with_time(id_class_1b,
                                         w,
                                         offset,
										 vibration_neai_output_probabilities,
        ...
        if (g_vneai_skip_first_window && (w == 0U)) {
            continue;
        }

        int leak_idx = (g_vibration_neai_leak_class_id >= 0 && g_vibration_neai_leak_class_id < (int)CLASS_NUMBER) ? g_vibration_neai_leak_class_id : 0;
        int bg_idx   = (g_vibration_neai_background_class_id   >= 0 && g_vibration_neai_background_class_id   < (int)CLASS_NUMBER) ? g_vibration_neai_background_class_id   : 1;

        float p_leak = vibration_neai_output_probabilities[leak_idx];
        float p_bg   = vibration_neai_output_probabilities[bg_idx];

        uint32_t center_samples = offset + (uint32_t)(DATA_INPUT_USER / 2U);
        uint32_t window_ms      = (sample_rate_hz > 0U) ? ((center_samples * 1000U) / sample_rate_hz) : 0U;

        uint32_t slot_idx = window_ms / VIBRATION_SLOT_DURATION_MS;
        ...
        if (p_leak >= VIBRATION_STRONG_LEAK_CONFIDENCE_THRESHOLD) {
    ...
    for (uint32_t s = 0U; s < slot_count; s++) {
        ...
        if (slot_average_leak_confidence >= VIBRATION_SLOT_LEAK_CONFIDENCE_THRESHOLD) {
            leaky_slot_count++;
        }
    }
    uint8_t slot_average_logic_triggered = (leaky_slot_count >= VIBRATION_MIN_LEAKY_SLOT_COUNT) ? 1U : 0U;

    float strong_leak_fraction = 0.0f;
    if (window_count > 0U) {
        strong_leak_fraction = (float)total_strong_leak_win / (float)window_count;
    }
    uint8_t strong_fraction_logic_triggered = (strong_leak_fraction >= VIBRATION_STRONG_LEAK_FRACTION_THRESHOLD) ? 1U : 0U;
    ...
    printf("[NEAI] Summary: 3s vibration chunk verdict = %s  decision logic = %s \r\n"
    ...
    lora_sendf("3s-Vib, %s, %.3f",verdict_str, (double)verdict_prob);
    HAL_Delay(200);
    // -----------------------------------------------
