...
# WAV -> raw patches -> compact 1D CNN -> TFLite
# ============================================================
...
def extract_patches_from_file(file_path):
    ...
    if len(wav) < PATCH_WAVE_SAMPLES:
        ...
        patches.append(compute_raw_patch_from_wave(padded))
        return patches

    start = 0
    while start + PATCH_WAVE_SAMPLES <= len(wav):
        chunk = wav[start:start + PATCH_WAVE_SAMPLES]
        patches.append(compute_raw_patch_from_wave(chunk))
    ...
    if start < len(wav):
        ...
        if len(tail) == PATCH_WAVE_SAMPLES:
            patches.append(compute_raw_patch_from_wave(tail))
...
def build_model():
    model = tf.keras.Sequential([
        tf.keras.layers.Input(shape=(PATCH_WAVE_SAMPLES, 1), name="raw_audio_input"),
...
def main():
    print("====================================================")
    print(" TF_audio_v4.1.py - Raw-audio CNN trainer ")
    print("====================================================\n")
