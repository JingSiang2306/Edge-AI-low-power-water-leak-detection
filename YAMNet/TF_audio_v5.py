...
# TF_audio_v5.py
# MFCC pipeline:
# WAV -> MFCC patches -> compact 2D CNN -> TFLite
# ============================================================

# -----------------------------
# USER CONFIG
# -----------------------------
DATASET_ROOT = "./dataset_YAMnet/acoustic"   # must contain /background and /leak
RANDOM_SEED = 42
...
# Audio / feature settings (YAMNet-style patching)
SR = 16000
N_MELS = 64
N_MFCC = 13
PATCH_FRAMES = 96
...
PATCH_WAVE_SAMPLES = WIN_LENGTH + (PATCH_FRAMES - 1) * HOP_LENGTH   # 15600 samples
PATCH_HOP_SECONDS = 0.48     # 50% overlap (0.96s patch, 0.48s hop)
PATCH_HOP_SAMPLES = int(PATCH_HOP_SECONDS * SR)
PATCH_SECONDS = PATCH_WAVE_SAMPLES / SR
...
def ask_version():
    """
    Ask user for version string like:
    v1.0, v1.5, v2.12, v3
    """
...
def make_workspace(version):
    workspace_dir = Path(f"./YAMnet_tflite_acoustic_{version}")
...
def split_file_list(file_list, train_ratio, val_ratio, test_ratio):
    """
    Split by file, not by patch, so patches from one file do not leak
    into multiple sets.
    """
...
def create_dataset_split(dataset_root, split_dir):
    """
    Copy files into:
      split_dir/train/background
      split_dir/train/leak
      split_dir/val/background
      split_dir/val/leak
      split_dir/test/background
      split_dir/test/leak
    ...
    for class_name in CLASS_MAPPING.keys():
        ...
        files = list_wav_files(class_folder)

        if len(files) == 0:
            raise RuntimeError(f"No .wav files found in {class_folder}")
...
def compute_full_logmel_from_wave(wav):
    ...
    return np.log(mel + 1e-3).astype(np.float32)


def compute_mfcc_patch_from_wave(wav):
    if len(wav) != PATCH_WAVE_SAMPLES:
        if len(wav) < PATCH_WAVE_SAMPLES:
            wav = np.pad(wav, (0, PATCH_WAVE_SAMPLES - len(wav)), mode="constant")
        else:
            wav = wav[:PATCH_WAVE_SAMPLES]
    ...
    if mfcc.shape[1] != PATCH_FRAMES:
        if mfcc.shape[1] < PATCH_FRAMES:
            mfcc = np.pad(mfcc, ((0, 0), (0, PATCH_FRAMES - mfcc.shape[1])), mode="constant")
        else:
            mfcc = mfcc[:, :PATCH_FRAMES]

    return mfcc.astype(np.float32)[..., np.newaxis]   # (N_MFCC, PATCH_FRAMES, 1)


def extract_patches_from_file(file_path):
    """
    Returns multiple MFCC patches from one wav file.
    ...
    if len(wav) < PATCH_WAVE_SAMPLES:
        padded = np.pad(wav, (0, PATCH_WAVE_SAMPLES - len(wav)), mode="constant")
        patches.append(compute_mfcc_patch_from_wave(padded))
        return patches

    start = 0
    while start + PATCH_WAVE_SAMPLES <= len(wav):
        chunk = wav[start:start + PATCH_WAVE_SAMPLES]
        patches.append(compute_mfcc_patch_from_wave(chunk))
        start += PATCH_HOP_SAMPLES

    # Ensure we also include the tail if the last window did not align exactly
    if start < len(wav):
        tail = wav[-PATCH_WAVE_SAMPLES:]
        if len(tail) == PATCH_WAVE_SAMPLES:
            patches.append(compute_mfcc_patch_from_wave(tail))

    return patches


def extract_patches_with_times_from_wave(wav):
    wav = wav.astype(np.float32)
    patches = []
    intervals = []
    ...
    while start + PATCH_WAVE_SAMPLES <= len(wav):
        chunk = wav[start:start + PATCH_WAVE_SAMPLES]
        patches.append(compute_mfcc_patch_from_wave(chunk))
        intervals.append((start / SR, (start + PATCH_WAVE_SAMPLES) / SR))
        start += PATCH_HOP_SAMPLES

    if start < len(wav):
        tail_start = max(0, len(wav) - PATCH_WAVE_SAMPLES)
        tail = wav[tail_start:tail_start + PATCH_WAVE_SAMPLES]
        if len(tail) == PATCH_WAVE_SAMPLES:
            tail_interval = (tail_start / SR, (tail_start + PATCH_WAVE_SAMPLES) / SR)
...
def build_dataset_from_split(split_root):
    """
    Build X, y from the copied split folder.
    Each file can contribute multiple patches.
    """
    X = []
    y = []
    stats = {
    ...
    for class_name, label in CLASS_MAPPING.items():
        ...
        wav_files = list_wav_files(class_dir)

        file_count = 0
        patch_count = 0

        for wav_file in wav_files:
            patches = extract_patches_from_file(wav_file)
...
def build_model():
    model = tf.keras.Sequential([
        tf.keras.layers.Input(shape=(N_MFCC, PATCH_FRAMES, 1), name="mfcc_input"),
...
def save_normalization_stats(mean, std, meta_dir, version):
    stats = {
        ...
        "std": std,
        "sr": SR,
        "patch_wave_samples": PATCH_WAVE_SAMPLES,
...
def export_tflite_models(model, X_train, model_dir, version):
    float_tflite_path = Path(model_dir) / f"YAMnet_audio_model_{version}_float32.tflite"
    int8_tflite_path = Path(model_dir) / f"YAMnet_audio_model_{version}_int8.tflite"
...
def visualize_test_file_predictions(model, file_path, true_class_name, mean, std, out_dir):
    wav, _ = librosa.load(file_path, sr=SR, mono=True)
    wav = wav.astype(np.float32)
    duration = len(wav) / SR if len(wav) > 0 else PATCH_SECONDS

    patches, intervals = extract_patches_with_times_from_wave(wav)
    ...
    full_logmel = compute_full_logmel_from_wave(wav)
    spec_time = np.arange(full_logmel.shape[1]) * (HOP_LENGTH / SR)
    waveform_time = np.arange(len(wav)) / SR
    patch_centers = np.array([(s + e) / 2.0 for s, e in intervals], dtype=np.float32)

    fig, axes = plt.subplots(3, 1, figsize=(14, 10), sharex=False)

    ax = axes[0]
    ax.plot(waveform_time, wav, linewidth=0.8)
    for s, e in highlight_intervals:
        ax.axvspan(s, e, alpha=0.25)
    ax.set_xlim(0, max(duration, PATCH_SECONDS))
    ax.set_title(
        f"Audio test validation: {Path(file_path).name}\n"
    ...
    ax = axes[1]
    extent = [0, spec_time[-1] if len(spec_time) > 0 else duration, 0, N_MELS]
    im = ax.imshow(full_logmel, aspect='auto', origin='lower', extent=extent)
    for s, e in highlight_intervals:
        ax.axvspan(s, e, alpha=0.20)
    ax.set_ylabel("Mel bin")
    ax.set_title("Full-file log-mel spectrogram")
    fig.colorbar(im, ax=ax, pad=0.01, label="Log-mel")
...
def run_test_validation_visualization(model, split_dir, mean, std, out_dir):
    ...
    for class_name in CLASS_MAPPING.keys():
        ...
        wav_files = list_wav_files(class_dir)
        for wav_file in wav_files:
            out_path = visualize_test_file_predictions(model, wav_file, class_name, mean, std, out_dir)
...
def main():
    print("====================================================")
    print(" TF_audio_v5.py - MFCC CNN trainer ")
    print("====================================================\n")
